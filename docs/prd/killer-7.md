...
FR-9
機能名: 配布更新チャネル（stable/canary）
説明: 管理PC向け配布では、Killer-7 Docker image をリリースタグ基準で更新できる。stable は最新正式版、canary は最新プレリリース（未存在時は stable）を解決する
優先度: Should

FR-10
機能名: 更新時ヘルスチェックとロールバック
...
- [ ] AC-13: 更新スクリプトが stable/canary のチャネル設定を解釈し、stable は最新正式タグ、canary は最新プレリリース（未存在時は stable）を対象にできる
